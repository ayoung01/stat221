...
sgd.imp <- function(data, alpha, plot=T) {
  # check.data(data)
  # Implements implicit
  n = nrow(data$X)
  p = ncol(data$X)
...
run.sgd.many <- function(nreps=100, alpha, n, length.out=10,
                         p=10, verbose=F, implicit=F) {
  ...
  return(sqrt.norm(empirical.var - Sigma.theoretical))
}
...
res = run.sgd.many(alpha=alpha, n=n)
write(res, file=sprintf("2e/sgd/%.0f_%.0f.txt", n, alpha))

res = run.sgd.many(alpha=alpha, n=n, implicit=T)
